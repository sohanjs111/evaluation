...
def fillpose(arg,frame_id):
       posCovMsg = PoseWithCovarianceStamped()	
       posCovMsg.header.frame_id = frame_id
       posCovMsg.header.stamp = arg[0]
       posCovMsg.pose.pose.position.x =arg[1]
       posCovMsg.pose.pose.position.y =arg[2]
       posCovMsg.pose.pose.position.z =arg[3]
       posCovMsg.pose.pose.orientation.x=arg[4]
       posCovMsg.pose.pose.orientation.y=arg[5]
       posCovMsg.pose.pose.orientation.z=arg[6]
       posCovMsg.pose.pose.orientation.w=arg[7]
       return posCovMsg
...
def writetospecfiletfandinital(file,inital,stamp,tfposoptitobase,tfposodomtobase,tfposoptitoodom):
	...
	with open (file,"a") as gt_pose_file:
		...
		gt_pose_file.write("\""+str(format(stamp,'.9f'))+", "+str(format(tfposoptitobase.position.x,'.11f'))+", "+str(format(tfposoptitobase.position.y,'.11f'))+", "+str(tfposoptitobase.position.z)+", "+str(format(tfposoptitobase.orientation.x,'.9f'))+", "+str(format(tfposoptitobase.orientation.y,'.9f'))+", "+str(format(tfposoptitobase.orientation.z,'.9f'))+", "+str(format(tfposoptitobase.orientation.w,'.9f'))+"\" \n")
		gt_pose_file.write("\""+str(format(stamp,'.9f'))+", "+str(format(tfposodomtobase.position.x,'.11f'))+", "+str(format(tfposodomtobase.position.y,'.11f'))+", "+str(tfposodomtobase.position.z)+", "+str(format(tfposodomtobase.orientation.x,'.9f'))+", "+str(format(tfposodomtobase.orientation.y,'.9f'))+", "+str(format(tfposodomtobase.orientation.z,'.9f'))+", "+str(format(tfposodomtobase.orientation.w,'.9f'))+"\" \n")
		gt_pose_file.write("\""+str(format(stamp,'.9f'))+", "+str(format(tfposoptitoodom.position.x,'.11f'))+", "+str(format(tfposoptitoodom.position.y,'.11f'))+", "+str(tfposoptitoodom.position.z)+", "+str(format(tfposoptitoodom.orientation.x,'.9f'))+", "+str(format(tfposoptitoodom.orientation.y,'.9f'))+", "+str(format(tfposoptitoodom.orientation.z,'.9f'))+", "+str(format(tfposoptitoodom.orientation.w,'.9f'))+"\" \n")
...
def gettransform():
       ...
       filltf(optitobase,gt_data[0],"opti","base_footprint",1)   
       #for test
       '''arg4=np.fromstring(sys.argv[4], dtype=float, sep=',')       
       maptodom=TransformStamped()	
       filltf(maptodom,arg4,"map","odom")'''
...
class Hector(object):
	...
	def writehector(self,posecov):
	    writeposetofile("hector_withouttf.txt",posecov,0)
...
class Ekf(object):
	def __init__(self,tfmaptoodom):
		...
		self.tfmaptoodom=tfmaptoodom
		self._first=0     
		self.star=TransformStamped()	   
	#callback for filtered ekf
	def writeEKFOdomToFile(self,ekf_odom_msg):
 		if self._first == 0:
		    	tfmrtp=generatetffrompose(ekf_odom_msg.pose,"map","pose")
		    	self.star=getinvers2(tfmrtp) 
			self.star.transform.translation.x=0
			self.star.transform.translation.y=0
			self.star.transform.translation.z=0
			self.star.transform.rotation.x=0
			self.star.transform.rotation.y=0
			self.star.transform.rotation.z=math.sin(0/2)
			self.star.transform.rotation.w=math.cos(0/2)			
	   		self._first = 1
 		pose_transformed = tf2_geometry_msgs.do_transform_pose(ekf_odom_msg.pose, self.star)	   
 		pose_transformed = tf2_geometry_msgs.do_transform_pose(pose_transformed, self.tfmaptoodom)	   
...
class Ohm(object):
	...
	def writeohm(self,posecov):
	    ...
	    angles = tf.transformations.euler_from_quaternion([postrans.pose.orientation.x, postrans.pose.orientation.y, postrans.pose.orientation.z, pose_transformed2.pose.orientation.w])
	    if self.first == 0:
...
if __name__ == "__main__":
    ...
    arg6=np.fromstring(sys.argv[9], dtype=float, sep=',')
    ...
    filltf(tfbasetocam,arg6,"base_footprint","camera_link",0)
    #for ekfcallback
    ekf=Ekf(tfmaptoodom)    
    #for orbslam
    ...
    GmapandAmclandKartoandMRPT = GmapandAmclandKartoandMRPT(sys.argv[7],tfmaptoodom,tfbasetolidar,mal)     
